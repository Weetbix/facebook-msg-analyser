    ...
    run: function( answers )
    {
        fs.readFile(answers.input, 'utf8', (err, data) => {
            ...
            let stats = {
                ...
                messageTypes  : _.uniqBy( _.flatten(messages.map( m => m.attachments)), 'type' ),
                messagesWithMoreThanOneAttachment : messages.some( m => m.attachments.length > 1 )
            }

            console.log( JSON.stringify(stats, null, 2) );
            fs.writeFileSync( 'stats.txt', JSON.stringify(stats, null, 2) );
        });
    }
}
